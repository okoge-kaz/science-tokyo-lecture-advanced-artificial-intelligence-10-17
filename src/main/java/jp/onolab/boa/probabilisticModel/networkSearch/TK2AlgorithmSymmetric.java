 ...
 * TK2Algorithm.javaとの違いは，ノード間のスコアの計算時に対称に計算を行っている点
 * @author shimazu
 */
public class TK2AlgorithmSymmetric extends TAbstractNetworkSearch {
    /** シリアライズ用のid **/
     ...
     * @param k 親ノード数の上限
     */
    public TK2AlgorithmSymmetric(int k) {
        super(k);
    }

    public TK2AlgorithmSymmetric(TK2AlgorithmSymmetric other) {
        super(other);
    }

    public TK2AlgorithmSymmetric clone() {
        return new TK2AlgorithmSymmetric(this);
    }
    ...
    public double search(TCIntMatrix network, List<List<Integer>> parentsOfNodes, IMetric metric, double minGain) {
        ...
        for(int i = 0; i < dim; ++i) {
            for(int j = i + 1; j < dim; ++j) {
                ...
                candidateGain.setValue(j, i, candidateGain.getValue(i, j));
            }
        }
